...
namespace BovineLabs.Quill.Debug.Physics
{
    ...
    public partial struct PhysicsDebugDrawSystem : ISystem
    {
        ...
        [ConfigVar("draw.physics-static_color", 0, 1, 0, 1, "Color used for static physics collider debug drawing.")]
        private static readonly SharedStatic<Color> StaticColor = SharedStatic<Color>.GetOrCreate<StaticColorType>();

        [ConfigVar("draw.physics-dynamics_color", 1, 0, 0, 1, "Color used for dynamic physics collider debug drawing.")]
        ...
        private unsafe struct DrawCollidersJob : IJobFor
        {
            ...
            {
                var vertex0 = math.transform(transform, collider->Vertex0);
                var vertex1 = math.transform(transform, collider->Vertex1);
                var position = (vertex1 + vertex0) / 2f;
                var radius = math.cmax(transform.Scale()) * collider->Radius;
                var height = math.distance(vertex0, vertex1) + (2 * radius);

                var axis = vertex1 - vertex0;
                var rotation = mathex.FromToRotation(math.up(), -axis);
            ...
            private void DrawTerrain(TerrainCollider* collider, float4x4 transform, Color color)
            {
                ...
                var numCells = terrain.Size - 1;
                var chunks = (numCells + (size - 1)) / size;
                ...
                {
                    var xMin = cx * size;
                    var xMax = math.min((cx + 1) * size, numCells.x);

                    for (var cy = 0; cy < chunks.y; cy++)
                    {
                        var yMin = cy * size;
                        var yMax = math.min((cy + 1) * size, numCells.y);
                        var aabb = CalculateTerrainChunkAabb(ref terrain, transform, xMin, xMax, yMin, yMax);
                        ...
                        var cellCount = (xMax - xMin) * (yMax - yMin);
                        var estCapacity = lineVertices.Length + (cellCount * linesPerSection);
            ...
            private static AABB CalculateTerrainChunkAabb(ref Terrain terrain, float4x4 transform, int xMin, int xMax, int yMin, int yMax)
            {
                ...
                for (var x = xMin; x <= xMax; x++)
                {
                    ...
                    {
                        var point = math.transform(transform, new float3(x, terrain.Heights[x + (terrain.Size.x * y)], y) * terrain.Scale);
