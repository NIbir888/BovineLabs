...
namespace BovineLabs.Quill
{
    ...
    public unsafe partial class DrawSystem : SystemBase
    {
        ...
        private const string DrawSceneEnabled = "draw.scene-switch";
        private const string DrawSceneDescEnabled = "Allow switching to scene view camera for culling when it has focus.";

        [ConfigVar(DrawSceneEnabled, true, DrawSceneDescEnabled)]
        ...
        protected override void OnDestroy()
        {
            ...
            foreach (var m in this.materials)
            {
                ...
                {
                    Object.DestroyImmediate(m.Material);
                }
                else
#endif
                {
                    Object.Destroy(m.Material);
                }
            }
        ...
        protected override void OnUpdate()
        {
            ...
            var previousTimedDrawer = this.timedDrawer;
            var timedEventWriter = new NativeThreadStream(Allocator.Persistent);
            this.Dependency = new DrawEventJob
            {
                Reader = previousTimedDrawer.AsReader(),
                MeshBuffers = this.builders,
            ...
            this.Dependency = previousTimedDrawer.Dispose(this.Dependency);
        ...
        private JobHandle GetAllDependencies(JobHandle handle)
        {
            foreach (var w in World.All)
            {
                ...
                {
                    handle = JobHandle.CombineDependencies(handle, w.Unmanaged.GetTrackedJobHandle());
                }
            }
        ...
        private void CreateDrawing()
        {
            ...
            this.timedDrawer = new NativeThreadStream(Allocator.Persistent);
        }
        ...
        private Camera GetCamera()
        {
            ...
            if (IsSceneEnabled.Data && sceneView != null)
            {
                var sceneHasFocus = EditorWindow.focusedWindow == sceneView;
...
#if BL_BRIDGE
            if (SystemAPI.QueryBuilder().WithAll<CameraBridge, CameraMain>().Build().TryGetSingleton<CameraBridge>(out var bridge) && bridge.Value.IsValid())
            {
        ...
        public struct Singleton : ISingletonCollection<Singleton.EnabledStream>
        {
            ...
            public Drawer CreateDrawer<T>(FixedString32Bytes category = default)
            {
                ...
                var enabled = IsEnabled.Data && this.Enabled.Value &&
                    (this.SystemFilterSet.Contains(systemTypeIndex) || (category != default(FixedString32Bytes) && this.CategoryFilterSet.Contains(category)));

                this.KnownSystemSet.Add(systemTypeIndex);

                if (category != default(FixedString32Bytes))
                {
        ...
        private struct DrawEventJob : IJobFor
        {
            ...
            [NativeDisableContainerSafetyRestriction]
            private NativeList<byte> readLargeBuffer2;
            ...
            public void Execute(int foreachIndex)
            {
                if (!this.readLargeBuffer.IsCreated)
                {
                    ...
                    this.readLargeBuffer2 = new NativeList<byte>(Allocator.Temp);
                }
                ...
                while (this.Reader.RemainingItemCount != 0)
                {
                    ...
                    switch (header.DrawType)
                    {
                        ...
                        case DrawType.Arrow:
                            ...
                            break;
                        case DrawType.BidirectionalArrow:
                            this.ExecuteDrawer<BidirectionalArrowDrawer>(header, ref this.Reader, ref buffer);
                            break;
                        case DrawType.Plane:
            ...
            private void ExecuteTrianglesDrawer(DrawHeader header, ref NativeThreadStream.Reader reader, ref DrawBuilder buffer)
            {
                ...
                {
                    this.readLargeBuffer2.ResizeUninitialized(drawer.Count * UnsafeUtility.SizeOf<Color>());
                    var colors = this.readLargeBuffer2.AsArray().Reinterpret<Color>(UnsafeUtility.SizeOf<byte>());
                    reader.ReadLarge<Color>((byte*)colors.GetUnsafePtr(), colors.Length);

                    for (var i = 0; i < triangles.Length; i++)
                    {
                        buffer.DrawTriangle(triangles[i].c0, triangles[i].c1, triangles[i].c2, colors[i]);
                    }
                    ...
                    if (duration > 0)
                    {
                        ...
                        this.TimedEventWriter.Write(drawer);
                        this.TimedEventWriter.WriteLarge(triangles);
                        this.TimedEventWriter.WriteLarge(colors);
                    }
                }
            }
        }
...
namespace BovineLabs.Quill
{
    ...
    public partial class DrawSystem : SystemBase
    {
        ...
        public struct Singleton : IComponentData
        {
            ...
            public CameraCulling CameraCulling => default;
