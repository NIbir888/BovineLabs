Shader "hidden/BovineLabs/Text"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // C# uploads through Unity.Rendering.SparseUploader, which requires
            // GraphicsBuffer.Target.Raw. Read it as a raw byte-address buffer here;
            // using StructuredBuffer<T> over a Raw target is backend/driver dependent
            // and on some D3D12/AMD drivers only one/partial glyph renders.
            ByteAddressBuffer text_buffer;

            text_data LoadTextData(uint index)
            {
                const uint stride = 128; // sizeof(text_data): 8 * float4
                const uint address = index * stride;

                text_data td;
                td.vertex0 = asfloat(text_buffer.Load4(address + 0));
                td.vertex1 = asfloat(text_buffer.Load4(address + 16));
                td.vertex2 = asfloat(text_buffer.Load4(address + 32));
                td.vertex3 = asfloat(text_buffer.Load4(address + 48));
                td.color = asfloat(text_buffer.Load4(address + 64));
                td.uv01 = asfloat(text_buffer.Load4(address + 80));
                td.uv23 = asfloat(text_buffer.Load4(address + 96));
                td.padding = asfloat(text_buffer.Load4(address + 112));
                return td;
            }
            ...
            v2f vert(uint vid : SV_VertexID)
            {
                float4 pos;
                float2 uv;
                const uint offset = (vid / 6) + _BaseVertex;
                const uint triVertex = vid % 6;

                // Expand each glyph quad into two triangles. MeshTopology.Quads is not
                // consistently supported on modern backends like D3D12/Metal/Vulkan,
                // which can make text render as only one/partial character on some GPUs.
                const uint vertex = triVertex == 0 ? 0u :
                    triVertex == 1 ? 1u :
                    triVertex == 2 ? 2u :
                    triVertex == 3 ? 0u :
                    triVertex == 4 ? 2u : 3u;

                const text_data td = LoadTextData(offset);
